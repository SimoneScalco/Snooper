...
if __name__ == "__main__":
    ...
    #################################################################################
    ##### TODO: CHANGE THE OWNERSHIP OF THE LOOT FOLDER
    #################################################################################
    ...
    if enable_module_cloud_enum:
        ...
        bash_command = ("cloud_enum -k {} -k {} -l {}").format(input_domain, input_domain_name, OUT_LOCAL_LOOT + OUT_FILE_CLOUD_ENUM)
    ...
    process = subprocess.Popen(bash_command.split(), stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding="utf-8")
    ...
    bash_command = ("sort -u {} -o {}").format(OUT_LOCAL_LOOT_SUBS + OUT_FILE_ALL_SUBS, OUT_LOCAL_LOOT_SUBS + OUT_FILE_ALL_SUBS_UNIQUE)
    ...
    bash_command = "httpx-toolkit -l {} -o {}".format(OUT_LOCAL_LOOT_SUBS + OUT_FILE_ALL_SUBS_UNIQUE, OUT_LOCAL_LOOT_SUBS + OUT_FILE_ALL_SUBS_UNIQUE_HTTPX)
    ...
    bash_command = "sudo naabu -l {} -tp 100 -o {} -json".format(OUT_LOCAL_LOOT_SUBS + OUT_FILE_ALL_SUBS_UNIQUE, OUT_LOCAL_LOOT + OUT_FILE_NAABU)
    process = subprocess.Popen(bash_command.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    output, error = process.communicate()
    ...
    for key in dict_naabu.keys():
        print("\n" + utils_module.get_inf_label() + "Ports found for host " + logger.bold_text(key) + " (total: " + str(len(dict_naabu[key])) + ")")
    ...
    bash_command = ("cp /usr/share/blackwidow/{}_80/{}_80-dynamic-unique.txt {}").format(input_domain, input_domain, OUT_LOCAL_LOOT + OUT_FILE_WEB_SPIDER)
    process = subprocess.Popen(bash_command.split(), stdout=subprocess.PIPE, bufsize=2)
    output, error = process.communicate()
